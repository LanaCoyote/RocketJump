...
function EFFECT:Init( cEffectData )
	...
	for i = 0, smokeSpriteCount do
		local period = 1 / (smokeSpriteCount - 12) * 2 * math.pi;

		local size = 96;
		local xAdjust = i < smokeSpriteCount - 12 and size * math.cos( i / period ) or 0;
		local yAdjust = i < smokeSpriteCount - 12 and size/2 * math.sin( i / period ) or 0;
		local zAdjust = i < smokeSpriteCount - 12 and 0 or size - math.random() * size * 2;

		local adj = Vector( size/2 - math.random() * size + xAdjust, size/2 - math.random() * size + yAdjust, size/2 - math.random() * size + zAdjust );
